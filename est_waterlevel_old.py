...
def est_lsu_water_level(seg_dir, out_dir, overlay_dir):
    ...
    for i in trange(len(seg_list)):
        ...
        if i == 0:
            gauge_pos = (seg == gauge_label_id).nonzero()
...
if __name__ == '__main__':
    ...
    img_dir = os.path.join(img_root, 'JPEGImages', args.video_name)
    seg_dir = os.path.join(seg_root, args.video_name)
    out_dir = os.path.join(out_root, args.video_name)
    gt_dir = os.path.join(img_root, 'WaterlevelGT', args.video_name)
    overlay_dir = os.path.join('overlay/AFB-URR_Water_fulltrain', args.video_name)

    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    if args.video_name == 'houston_buffalo_20170825_20170901_s':
        water_label_id = 1
